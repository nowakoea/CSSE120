...
def hourglass(window, n, point, radius, color):
    ...
    """
    # ------------------------------------------------------------------
    # DONE:      We provided some tests for you (above).
    # ------------------------------------------------------------------
    ####################################################################
    ...
    for j in range(n):  # Loop through the rows
        ...
        x = original_x - (radius * (j + 1))  # Reset x to the left-edge,
        # for the next
        #  row
...
def many_hourglasses(window, square, m, colors):
    ...
    # ------------------------------------------------------------------
    original_x = square.center.x
    original_y = square.center.y

    x = original_x
    y = original_y
    n = 0

    for j in range(m):  # Loop through the rows
        for _ in range(j + 1):  # Loop through the columns
            new_rect = rg.Rectangle(rg.Point(square.center.x +
                                             square.length_of_each_side / 2,
                                             square.center.y -
                                             square.length_of_each_side / 2),
                                    rg.Point(square.center.x + (3 *
                                                                square.length_of_each_side / 2,
                                                                square.center.y + square.length_of_each_side / 2)))
            new_rect.attach_to(window.initial_canvas)
            window.render(0.1)
            n = n + 1
            hourglass(window, n, square.center,
                      square.length_of_each_side / n, colors[j])
            if j == len(colors):
                colors = colors + colors[j] + ''
